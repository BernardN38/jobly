...
const { ensureLoggedIn , checkUserorAdmin, checkAdmin } = require("../middleware/auth");
...
router.patch("/:username", ensureLoggedIn, checkUserorAdmin, async function (req, res, next) {
  try {
...
router.post("/:username/jobs/:id", ensureLoggedIn, async function (req, res, next) {
  try {
    ...
    return res.json({ applied: job.job_id });
  } catch (err) {
    return next(err);
  }
});
...
router.delete("/:username", ensureLoggedIn, checkUserorAdmin,  async function (req, res, next) {
  try {
